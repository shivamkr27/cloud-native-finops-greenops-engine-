...
docker build -t carbon-scheduler:local cron-scheduler
...
minikube -p $Profile image load carbon-scheduler:local
...
(Get-Content cron-scheduler/cronjob.yaml) -replace 'carbon-scheduler:latest', 'carbon-scheduler:local' | kubectl apply -f -
